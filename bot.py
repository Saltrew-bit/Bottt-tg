...
async def rules(callback: types.CallbackQuery):
    await callback.message.answer(
        text=(
            ...
            "2. Реальная цена (только цифры, например 450.000)\n"
            "3. Контакт обязателен\n"
            "4. Подробное описание автомобиля\n"
            "5. Фото — до 10 шт."
        ),
        parse_mode="Markdown"
    )
...
async def process_message(msg: types.Message):
    ...
    # Шаг 2: год выпуска
        ...
        if not msg.text.isdigit():
            await msg.answer("Ошибка! Введите год цифрами, например: 2015")
            return
        ad["year"] = msg.text
        ads_data[user_id]["step"] = 3
        await msg.answer("Введите цену в ₽ (только цифры, например 450000):")
    # Шаг 3: цена
    elif step == 3:
        if not msg.text.replace(".", "").isdigit():
            await msg.answer("Ошибка! Введите цену цифрами, например 450.000")
            return
        ad["price"] = msg.text
    ...
    # Шаг 4: пробег
        ...
        if not msg.text.isdigit():
            await msg.answer("Ошибка! Введите пробег цифрами, например 120000")
            return
        ad["mileage"] = msg.text
