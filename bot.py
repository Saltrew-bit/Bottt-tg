...
async def rules(callback: types.CallbackQuery):
    await callback.message.answer(
        ...
        "4. Фото автомобиля до 10 штук\n"
        "5. Краткое описание желательны информативное\n\n"
        "Соблюдайте правила, чтобы объявление не отклонили.",
        parse_mode="Markdown"
    )
...
async def add_ad(callback: types.CallbackQuery):
    ...
    await callback.message.answer("🚗 Введите марку и модель автомобиля (например: Lada Vesta):")
...
async def process_message(msg: types.Message):
    ...
    # Шаг 3: цена
        ...
        await msg.answer("📏 Введите пробег в километрах (только цифры, например: 120000):")
    ...
    # Шаг 7: описание и предпросмотр
        ...
        pending_ads[user_id] = ad  # сохраняем данные для кнопок
        ...
        # Отправляем текст предпросмотра с кнопками
        await msg.answer(preview_text, reply_markup=preview_keyboard, parse_mode="Markdown")
        ads_data[user_id]["step"] = 8  # новый "шаг предпросмотра"
...
async def handle_preview_buttons(cq: types.CallbackQuery):
    ...
    if cq.data.startswith("moderate_"):
        ...
        if ad:
            ...
            await cq.message.edit_reply_markup()  # убираем кнопки у пользователя
        ...
        await cq.answer("Вы отменили подачу объявления.")

# --- Действия админа для публикации ---
