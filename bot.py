...
@dp.callback_query_handler(lambda c: c.data == "new_ad")
...
async def publish_ad(cq: types.CallbackQuery):
...
async def delete_ad(cq: types.CallbackQuery):
    if cq.from_user.id != ADMIN_ID:
        await cq.answer("Только админ может управлять объявлениями.")
        return
    user_id = int(cq.data.split("_")[1])
