...
user_ads = {}  # В процессе заполнения
pending_ads = {}  # На модерации
...
async def add_ad(callback: types.CallbackQuery):
    user_ads[callback.from_user.id] = {"step": 1, "data": {}}
...
async def process_ads(msg: types.Message):
    ...
    if user_id not in user_ads:
        return
    ad = user_ads[user_id]["data"]
    step = user_ads[user_id]["step"]
    ...
    if step == 1:
        ...
        if msg.photo:
            ...
            if len(ad["photos"]) < 10:
                ...
                await msg.answer("Максимум фото достигнут. Введите описание автомобиля:")
        ...
        await msg.answer("Введите контакт (телефон или Telegram):")
    elif step == 7:
        ad["contact"] = msg.text
        user_ads[user_id]["step"] = 8
...
async def admin_actions(callback: types.CallbackQuery):
    ...
    if callback.data.startswith("publish_") and ad:
        text = (
            f"🚗 {ad['model']}\n"
            f"📅 {ad['year']}\n"
            f"💰 {ad['price']} ₽\n"
