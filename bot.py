...
async def process_message(msg: types.Message):
    ...
    if step == 1:
        ...
        await msg.answer("Введите контакт (номер или Telegram):")
    elif step == 6:
        ad["contact"] = msg.text
        ads_data[user_id]["step"] = 7
        await msg.answer("Теперь отправьте фото автомобиля (до 10). Когда закончите — напишите 'стоп'.")
    elif step == 7:
        if msg.photo:
            ...
            if len(ad["photos"]) < 10:
                ...
                await msg.answer("Фото завершены. Ваше объявление готово к модерации.")
...
async def handle_admin_actions(cq: types.CallbackQuery):
    ...
    if data.startswith("publish_") and ad:
        ...
        if media:
            await bot.send_media_group(chat_id="@AutoHub62Channel", media=media)
        await bot.send_message(chat_id="@AutoHub62Channel", text=text)
